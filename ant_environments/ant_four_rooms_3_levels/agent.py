...
class Agent():
    ...
    def initialize_networks(self):
        ...
        vars_to_restore=[v for v in tf.trainable_variables() if ("actor" in v.name or "critic" in v.name) and ("actor_3" not in v.name and "critic_3" not in v.name) and ("actor_2" not in v.name and "critic_2" not in v.name)]
